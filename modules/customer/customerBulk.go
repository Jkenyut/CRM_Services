...
func CustomerBulk(c *gin.Context) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, entity.DefaultErrorResponseWithMessage("uri error", http.StatusBadRequest))
		return
	}
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, entity.DefaultErrorResponseWithMessage("data decode error", http.StatusBadRequest))
		return
	}
	...
	for _, customer := range responseUri.Data {
		...
		//check
		if result.Error != nil {
			c.AbortWithStatusJSON(http.StatusBadRequest, entity.DefaultErrorResponseWithMessage("failed exec query create bulk actor", http.StatusBadRequest))
			break
		}
	}
