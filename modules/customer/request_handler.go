...
func (h RequestHandlerCustomerStruct) ActivateCustomerById(c *gin.Context) {
	customerId, err := strconv.ParseUint(c.Param("id"), 10, 64)
	res, err := h.ctr.ActivateCustomerById(uint(customerId))
	if err != nil {
		if err.Error() == "customer not found" {
			c.JSON(http.StatusNotFound, dto.DefaultErrorResponseWithMessage("Customer not found"))
			return
...
func (h RequestHandlerCustomerStruct) DeactivateCustomerById(c *gin.Context) {
	customerId, err := strconv.ParseUint(c.Param("id"), 10, 64)
	res, err := h.ctr.DeactivateCustomerById(uint(customerId))
	if err != nil {
		if err.Error() == "customer not found" {
			c.JSON(http.StatusNotFound, dto.DefaultErrorResponseWithMessage("Customer not found"))
			return
		} else if err.Error() == "customer is super admin can't deactivate" {
			c.JSON(http.StatusUnauthorized, dto.DefaultErrorResponseWithMessage("customer is super admin can't deactivate"))
...
//func (h RequestHandlerCustomerStruct) LoginCustomer(c *gin.Context) {
//	request := CustomerBody{}
//	err := c.Bind(&request)
//	fmt.Println(request, err)
//	if err != nil {
...
//	}
//
//	res, err := h.ctr.LoginCustomer(request)
//	if err != nil {
//		if err.Error() == "username already taken" {
//			c.JSON(http.StatusConflict, dto.DefaultErrorResponseWithMessage("Username already taken"))
//			return
